...
namespace PaymentTerminalService.Model
{
    ...
    public abstract partial class PaymentTerminalBase
    {
        ...
        private DateTimeOffset? lastFinalStatusAt;
        ...
        /// <summary>
        /// Acquires the session lock and delegates to <see cref="ValidateCanStartOperation"/>.
        /// Throws if the requested operation cannot be started in the current state.
        /// </summary>
        /// <param name="operationType">The operation type being requested.</param>
        ...
        /// <summary>
        /// Validates whether the requested operation can start given the current terminal state and session.
        /// Must be called while holding <see cref="sessionStatusesLock"/>.
        /// <para>
        /// Rules per operation type:
        /// <list type="bullet">
        /// <item><description>
        /// <see cref="OperationType.Abort"/> — allowed whenever a session is active or the terminal
        /// is not already idle after a completed abort.
        /// </description></item>
        /// <item><description>
        /// <see cref="OperationType.Purchase"/> — allowed without the standard busy check when
        /// transitioning directly from a completed loyalty activation (<see cref="TerminalState.AwaitingResult"/>).
        /// </description></item>
        /// <item><description>
        /// <see cref="OperationType.LoyaltyActivate"/> / <see cref="OperationType.LoyaltyDeactivate"/> —
        /// requires <see cref="IsLoyaltySupported"/>; blocked while a session is active or terminal is busy.
        /// </description></item>
        /// <item><description>
        /// <see cref="OperationType.Refund"/> — requires <see cref="IsRefundSupported"/>;
        /// blocked while a session is active or terminal is busy.
        /// </description></item>
        /// <item><description>
        /// <see cref="OperationType.Reversal"/> — requires <see cref="IsReversalSupported"/>;
        /// blocked while a session is active or terminal is busy.
        /// </description></item>
        /// <item><description>
        /// All other operations — blocked while a session is active or terminal is busy.
        /// </description></item>
        /// </list>
        /// </para>
        /// </summary>
        /// <param name="operationType">The operation type being requested.</param>
        /// <param name="status">The current terminal status snapshot.</param>
        /// <param name="isSessionActive">Whether a payment session is currently active.</param>
        /// <exception cref="NotSupportedException">
        /// Thrown when the operation requires a capability that is not enabled for this terminal.
        /// </exception>
        /// <exception cref="ApiConflictException">
        /// Thrown when the operation cannot start due to an active session or a busy terminal state.
        /// </exception>
        private void ValidateCanStartOperation(OperationType operationType, TerminalStatus status, bool isSessionActive)
        {
            ...
            {
                if (!IsRefundSupported)
                    throw new NotSupportedException("Refund operations are not supported by this terminal.");
            ...
            else if (operationType == OperationType.Reversal)
            {
                if (!IsReversalSupported)
                    throw new NotSupportedException("Reversal operations are not supported by this terminal.");
        ...
        protected void UpdateCurrentStatus(TerminalStatus status)
        {
            ...
            try
            {
                lock (sessionStatusesLock)
                {
                    ...
                    // Automatically populate PreviousState from currentStatus
                    if (currentStatus != null)
                    {
                        status.PreviousState = currentStatus.State;
                    }
                    ...
                    if (status.LastResultIsFinal)
                        lastFinalStatusAt = DateTimeOffset.UtcNow;
        ...
        /// <summary>
        /// Persists a <see cref="TerminalStatus"/> that arrived outside any managed session
        /// as an orphan record for manual review or reconciliation.
        /// </summary>
        /// <remarks>
        /// Exceptions are caught and traced rather than propagated, consistent with the
        /// event-handler safety contract used by <see cref="AddSessionStatus"/> and
        /// <see cref="UpdateCurrentStatus"/>.
        /// </remarks>
        /// <param name="status">The unrecognized status to persist.</param>
        private void SaveOrphan(TerminalStatus status)
        {
            Trace.WriteLine($"{nameof(SaveOrphan)}: {status}", GetType().FullName);

            try
            {
                lock (sessionPersistenceLock)
                {
                    SessionStorageProvider.SaveOrphan(status);
                }
            }
            catch (Exception ex)
            {
                Trace.WriteLine($"{nameof(SaveOrphan)}: Could not save orphan: {ex}", GetType().FullName);
            }
        }

        /// <summary>
        /// Handles a terminal status that arrived outside any managed session by updating the
        /// current in-memory status and persisting it as an orphan record for manual review
        /// or reconciliation. Override to customize orphan handling in a derived implementation.
        /// </summary>
        /// <remarks>
        /// When a payment session is active the in-memory state is owned by the session —
        /// <see cref="UpdateCurrentStatus"/> is skipped and the orphan is only persisted.
        /// </remarks>
        /// <param name="status">The unrecognized status to handle.</param>
        protected virtual void HandleOrphanStatus(TerminalStatus status)
        {
            Trace.WriteLine($"{nameof(HandleOrphanStatus)}: {status}", GetType().FullName);
            ...
            if (!IsPaymentSessionActive)
                ...
                Trace.WriteLine($"{nameof(HandleOrphanStatus)}: session active, skipping UpdateCurrentStatus for orphan operationId={status.ActiveOperationId}.", GetType().FullName);
