...
namespace PaymentTerminalService.Model
{
    ...
    /// <item><term>Orphan</term><description><c>2026-03-05_14-22-10.123.orphan.ptss</c></description></item>
    /// </list>
    ...
    public sealed class FileSessionStorageProvider : ISessionStorageProvider
    {
        ...
        /// failed file name and the ongoing file is deleted.
        /// </para>
        /// </remarks>
        ...
        /// <inheritdoc/>
        /// <remarks>
        /// Wraps <paramref name="status"/> in a synthetic <see cref="TerminalSessionResponse"/> so
        /// that orphan files share the same format and load path as all other session files.
        /// </remarks>
        public void SaveOrphan(TerminalStatus status)
        {
            ...
            var sessionName = sessionNameProvider.GetOrCreateSessionName(null);

            var item = new TerminalSessionResponse
            {
            ...
            var fileName = sessionNameProvider.GetOrphanSessionFileName(sessionName);
            var filePath = GetSessionFilePath(fileName);
            var payload = JsonConvert.SerializeObject(item, Formatting.Indented);
        ...
        public void ConfirmSession(string sessionName)
        {
            ...
            var completedPath = GetSessionFilePath(sessionNameProvider.GetCompletedSessionFileName(sessionName));
            var failedPath = GetSessionFilePath(sessionNameProvider.GetFailedSessionFileName(sessionName));
            ...
            {
                // A failed session is only confirmable when its final state represents a known
                // business outcome (e.g. declined, card expired — State=Idle).
                // Infrastructure faults (State=Error: connection lost, hardware failure) leave
                // the transaction outcome unknown and must not be confirmed.
                var session = JsonConvert.DeserializeObject<TerminalSessionResponse>(File.ReadAllText(failedPath));
                var latestStatus = GetLatestStatus(session);

                if (latestStatus == null || latestStatus.State == TerminalState.Error)
                    throw new InvalidOperationException(
                        $"Session '{sessionName}' ended with an infrastructure fault (State={latestStatus?.State}) and cannot be confirmed. The transaction outcome is unknown.");

                sourcePath = failedPath;
            }
            else
            {
                throw new KeyNotFoundException($"No completed or failed session file found for session '{sessionName}'.");
            }
        ...
        public IEnumerable<TerminalSessionResponse> LoadCompletedSessions()
        {
            return LoadSessionsFromDirectory(
                ...
                excludeSuffix3: TimestampFileNameProvider.OrphanSuffix + TimestampFileNameProvider.FileExtension);
        }
        ...
        /// <inheritdoc/>
        public IEnumerable<TerminalSessionResponse> LoadOrphanSessions()
        {
            return LoadSessionsFromDirectory(
                "*" + TimestampFileNameProvider.OrphanSuffix + TimestampFileNameProvider.FileExtension);
        }
            ...
            string excludeSuffix3 = null)
        {
            ...
            foreach (var filePath in files)
            {
                ...
                if (excludeSuffix3 != null && fileName.EndsWith(excludeSuffix3))
                    continue;
