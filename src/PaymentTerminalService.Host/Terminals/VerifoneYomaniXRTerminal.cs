...
namespace PaymentTerminalService.Terminals
{
    ...
    internal sealed class VerifoneYomaniXRTerminal : PaymentTerminalBase, IUserPromptHandler
    {
        ...
        /// </summary>
        private IECRTerminalManager TerminalManager { get; }
        ...
        {
            TraceSerialBytes = vendorPayload?.AdditionalProperties.GetPropertyOrDefault<bool>(VerifoneYomaniXRVendorPayloadKeys.TraceSerialBytes) ?? false;

            EnableManualAuthorization = vendorPayload?.AdditionalProperties.GetPropertyOrDefault<bool>(VerifoneYomaniXRVendorPayloadKeys.EnableManualAuthorization) ?? false;
        ...
        private void TerminalManager_TransactionInitialized(object sender, TransactionEventArgs e)
        {
            ...
            var vendorPayload = new VendorPayload
            {
                ...
                {
                    [VendorPayloadKeys.TransactionId] = e.TransactionId,
                    [VendorPayloadKeys.TransactionDateTime] = e.TransactionDateTime,
                }
            };
            ...
            else
            {
                ...
                // Surfaced as a diagnostic status so the POS can detect unexpected terminal activity.
                UpdateCurrentStatus(new TerminalStatus
                {
                    State = TerminalState.TransactionInProgress,
                    IsConnected = currentStatus.IsConnected,
        ...
        private void TerminalManager_AbortTransactionResultReceived(object sender, AbortTransactionResultEventArgs e)
        {
            ...
            else
            {
                ...
                // If no session is active, this is a stale rejection from a session already
                // closed by a forced abort; discard it to avoid overwriting the final status
                // with a non-final one, which would cause the POS to poll indefinitely.
                if (!isSessionActive)
                {
                    Trace.WriteLine($"{nameof(TerminalManager_AbortTransactionResultReceived)}: abort rejected outside active session — discarding stale response.", GetType().FullName);
                    return;
                }
        ...
        private VendorPayload BuildTransactionVendorPayload(TransactionResultExEventArgs transactionInfo, string transactionFilePath, string clientReference)
        {
            ...
            payload.AdditionalProperties[VendorPayloadKeys.TransactionId] = transactionInfo.TransactionId;
            ...
            payload.AdditionalProperties[VendorPayloadKeys.TransactionDateTime] = transactionInfo.TransactionDateTime;
        ...
        private void DeleteTransactionFile(string transactionFilePath)
        {
            ...
            {
                Trace.WriteLine($"{nameof(DeleteTransactionFile)}:\n{ex}", GetType().FullName);
            }
        }
        ...
        private void HandleTransactionResult(TransactionResultEventArgs e, bool isOutsideTerminalManagerSession)
        {
            ...
            {
                HandleOrphanStatus(new TerminalStatus
                {
        ...
        /// <inheritdoc/>
        protected override async Task DoAbortTransactionAsync(AbortTransactionRequest request, CancellationToken cancellationToken)
        {
            Trace.WriteLine($"{nameof(DoAbortTransactionAsync)}", GetType().FullName);
                ...
                new TerminalSettings
                {
                    VendorPayload = VendorPayload
                        ?? new VendorPayload
                        {
                            ...
                            {
                                { VerifoneYomaniXRVendorPayloadKeys.TraceSerialBytes, TraceSerialBytes },
                                { VerifoneYomaniXRVendorPayloadKeys.EnableManualAuthorization, EnableManualAuthorization }
                            }
                        }
                });
        }
