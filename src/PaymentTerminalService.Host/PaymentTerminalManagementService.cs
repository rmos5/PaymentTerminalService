...
namespace PaymentTerminalService.Host
{
    ...
    internal partial class PaymentTerminalManagementService : IPaymentTerminalSelector, IDisposable
    {
        ...
        private readonly string terminalSessionDirectory;
            ...
            Terminals = new List<TerminalDescriptor>
            {
                new TerminalDescriptor
                {
                    ...
                    VendorPayload = new VendorPayload
                    {
                        ...
                        {
#if DEBUG
                            { VendorPayloadKeys.IsRefundSupported, true },
#else
                            { VendorPayloadKeys.IsRefundSupported, false },
#endif
                            { VendorPayloadKeys.IsReversalSupported, true }
                        }
                    },
        ...
        /// </summary>
        /// <param name="terminalSessionDirectory">
        /// Base directory under which per-terminal session storage subdirectories are created.
        /// </param>
        public PaymentTerminalManagementService(string terminalSessionDirectory)
        {
            lock (sync)
            {
            ...
            this.terminalSessionDirectory = terminalSessionDirectory;
        }
        ...
        /// that exist in both sources; catalog-definition <see cref="VendorPayload"/> keys are
        /// always re-applied as the baseline so new definition keys are never missing after an upgrade.
        /// Terminals absent from the definition are dropped.
        /// If the file is missing or cannot be parsed, a deep clone of the static definition is used.
        ...
        private TerminalCatalogResponse LoadOrCreateCatalog()
        {
            ...
            try
            {
                if (File.Exists(CatalogFileName))
                {
                    ...
                    if (catalog != null && catalog.Terminals != null)
                    {
                        ...
                        // Building new catalog, preserving settings from serialized file where possible
                        ...
                        foreach (var def in defTerminals.Values)
                        {
                            ...
                            {
                                // Preserve user settings from file, but ensure catalog-definition
                                // VendorPayload keys are always present as the baseline.
                                existing.VendorPayload = MergeVendorPayload(def.VendorPayload, existing.VendorPayload);
                                mergedTerminals.Add(existing);
            ...
            {
                Trace.WriteLine($"{nameof(LoadOrCreateCatalog)}:\n{ex}", GetType().FullName);
            }

            // Fallback: deep-clone the static definition so mutations never affect it.
            Trace.WriteLine("Using static catalog definition and creating file", GetType().FullName);
            var fallback = CloneCatalogDefinition();
            SaveCatalog(fallback);
            return fallback;
        }

        /// <summary>
        /// Returns a deep clone of <see cref="catalogDefinition"/> by round-tripping through JSON.
        /// Called on the fallback path in <see cref="LoadOrCreateCatalog"/> to ensure the live catalog
        /// is always a separate object from the static definition and cannot be accidentally mutated.
        /// </summary>
        private static TerminalCatalogResponse CloneCatalogDefinition()
        {
            var json = JsonConvert.SerializeObject(catalogDefinition, Formatting.None);
            return JsonConvert.DeserializeObject<TerminalCatalogResponse>(json);
        }
        ...
        private void SaveCatalog(TerminalCatalogResponse catalog)
        {
            ...
            {
                Trace.WriteLine($"{nameof(SaveCatalog)}:\n{ex}", GetType().FullName);
            }
        }
        ...
        private int ComputeSelectionHash(string terminalId, string connectionId, TerminalConnectionOption connection, bool isLoyaltySupported, VendorPayload terminalVendorPayload)
        {
            unchecked
            {
                ...
                hash = hash * 23 + (connection?.VendorPayload != null ? ComputeOrderedPayloadHash(connection.VendorPayload) : 0);
                hash = hash * 23 + (terminalVendorPayload != null ? ComputeOrderedPayloadHash(terminalVendorPayload) : 0);
                return hash;
            }
        }

        /// <summary>
        /// Computes an ordered, collision-resistant hash over a <see cref="VendorPayload"/>'s
        /// entries by sorting keys before folding, avoiding the self-cancellation
        /// that XOR-based order-independent hashes produce for symmetric value sets.
        /// </summary>
        /// <param name="payload">The payload to hash; must not be null.</param>
        /// <returns>A hash that is sensitive to both key identity and value.</returns>
        private static int ComputeOrderedPayloadHash(VendorPayload payload)
        {
            unchecked
            {
                int hash = 17;
                foreach (var key in payload.AdditionalProperties.Keys.OrderBy(k => k, StringComparer.Ordinal))
                {
                    hash = hash * 23 + StringComparer.Ordinal.GetHashCode(key);
                    hash = hash * 23 + (payload.AdditionalProperties[key]?.GetHashCode() ?? 0);
                }
                return hash;
            }
        }
        ...
        /// <summary>
        /// Merges <paramref name="requestPayload"/> on top of <paramref name="catalogPayload"/>,
        /// returning a new <see cref="VendorPayload"/> where catalog-defined keys are preserved
        /// and any keys supplied by the request override or extend them.
        /// If <paramref name="requestPayload"/> is null or empty, a copy of the catalog payload is returned.
        /// </summary>
        /// <param name="catalogPayload">The baseline payload, typically from <see cref="catalogDefinition"/>.</param>
        /// <param name="requestPayload">The caller-supplied payload to merge in; may be null.</param>
        /// <returns>A new <see cref="VendorPayload"/> instance; never null.</returns>
        private VendorPayload MergeVendorPayload(VendorPayload catalogPayload, VendorPayload requestPayload)
        {
            var merged = new VendorPayload();

            if (catalogPayload != null)
            {
                foreach (var entry in catalogPayload.AdditionalProperties)
                    merged.AdditionalProperties[entry.Key] = entry.Value;
            }

            if (requestPayload != null)
            {
                foreach (var entry in requestPayload.AdditionalProperties)
                    merged.AdditionalProperties[entry.Key] = entry.Value;
            }
        ...
        public async Task<SelectedTerminalResponse> SelectTerminalAsync(SelectTerminalRequest request, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                VendorPayload mergedVendorPayload = null;
                int hash = 0;
                ...
                lock (sync)
                {
                    ...
                    // Merge request payload on top of the *static* definition baseline — not the
                    // live catalog entry — so catalog-definition keys are always authoritative
                    // and cannot be eroded by accumulated prior selections.
                    var defTerminal = catalogDefinition.Terminals.FirstOrDefault(t => t.TerminalId == request.TerminalId);
                    mergedVendorPayload = MergeVendorPayload(defTerminal?.VendorPayload, request.VendorPayload);
                    ...
                    // Capture and clear the current terminal now so its port is released
                    // before the new instance attempts to open the same resource.
                    previousTerminal = selectedPaymentTerminal;
                    selectedPaymentTerminal = null;
                    lastSelectionHash = 0;
                }

                // Release and dispose the previous terminal before opening the port on the new one.
                ...
                newTerminal = CreateTerminalInstance(terminal, request.Connection, request.IsLoyaltySupported, mergedVendorPayload);
                ...
                try
                {
                    ...
                    {
                        Trace.WriteLine($"{nameof(SelectTerminalAsync)}:\n{ex2}", GetType().FullName);
                    }

                    terminal.Version = newTerminal.Version; // Update catalog with actual terminal info after successful test.
                }
                catch (Exception ex)
                {
                    Trace.WriteLine($"{nameof(SelectTerminalAsync)}:\n{ex}", GetType().FullName);
                ...
                lock (sync)
                {
                    ...
                    terminal.VendorPayload = mergedVendorPayload;
                ...
                return await GetSelectedTerminalAsync(cancellationToken).ConfigureAwait(false);
            }
            finally
            {
        ...
        /// <inheritdoc/>
        public async Task<OperationAccepted> DeselectTerminalAsync(CancellationToken cancellationToken = default)
        {
            Trace.WriteLine($"{nameof(DeselectTerminalAsync)}", GetType().FullName);
        ...
        /// <summary>
        /// Creates and returns a new terminal instance for the given descriptor and connection,
        /// using the provided effective loyalty and vendor payload values.
        /// </summary>
        /// <param name="terminal">Terminal descriptor identifying the implementation to instantiate.</param>
        /// <param name="connection">Connection option to pass to the terminal constructor.</param>
        /// <param name="isLoyaltySupported">Effective loyalty flag for this selection.</param>
        /// <param name="vendorPayload">Effective vendor payload for this selection.</param>
        /// <returns>A new <see cref="IPaymentTerminal"/> instance ready for connection testing.</returns>
        /// <exception cref="ApiBadRequestException">
        /// Thrown when <paramref name="terminal"/> or <paramref name="connection"/> is null,
        /// or when the terminal ID is not recognised.
        /// </exception>
        private IPaymentTerminal CreateTerminalInstance(TerminalDescriptor terminal, TerminalConnectionOption connection, bool isLoyaltySupported, VendorPayload vendorPayload)
        {
            if (terminal == null)
            ...
            if (terminal.TerminalId == VerifoneYomaniXR_001)
            {
                return new VerifoneYomaniXRTerminal(
                    ...
                    isLoyaltySupported,
                    vendorPayload,
                    new FileSessionStorageProvider(Path.Combine(terminalSessionDirectory, terminal.TerminalId), new TimestampFileNameProvider()));
            }
        ...
        /// <summary>
        /// Attempts to silently restore the previously persisted terminal selection on startup.
        /// If the catalog has a <see cref="TerminalCatalogResponse.SelectedTerminalId"/> and a
        /// matching <see cref="TerminalDescriptor.SelectedConnectionId"/>, creates and tests the
        /// terminal instance. On success the instance is installed as <see cref="selectedPaymentTerminal"/>.
        /// On any failure the error is traced and the selection is left as unresolved — the catalog
        /// is never modified, so the persisted selection remains available for the next explicit
        /// <see cref="SelectTerminalAsync"/> call.
        /// </summary>
        internal async Task TryRestoreSelectionAsync()
        {
            Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}", GetType().FullName);
            ...
            lock (sync)
            {
                ...
                if (string.IsNullOrWhiteSpace(selectedId))
                {
                    Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}: no persisted selection, skipping.", GetType().FullName);
                    return;
                }

                terminal = terminalCatalogResponse.Terminals?.FirstOrDefault(t => t.TerminalId == selectedId);
                if (terminal == null)
                {
                    Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}: persisted terminal '{selectedId}' not found in catalog, skipping.", GetType().FullName);
                    return;
                }

                connection = terminal.Connections?.FirstOrDefault(c => c.ConnectionId == terminal.SelectedConnectionId)
                    ?? terminal.Connections?.FirstOrDefault();

                if (connection == null)
                {
                    Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}: no connection found for terminal '{selectedId}', skipping.", GetType().FullName);
                    return;
                }
                ...
                restoredHash = ComputeSelectionHash(
                    terminal.TerminalId,
            ...
            Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}: restoring terminal '{terminal.TerminalId}' connection '{connection.ConnectionId}'.", GetType().FullName);

            IPaymentTerminal restoredTerminal = null;
            try
            {
                ...
                {
                    Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}:\n{ex2}", GetType().FullName);
                }

                lock (sync)
                {
                    // Only commit if no explicit selection was made concurrently while we were testing.
                    if (selectedPaymentTerminal == null)
                    {
                        selectedPaymentTerminal = restoredTerminal;
                        lastSelectionHash = restoredHash;
                        restoredTerminal = null;
                        Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}: terminal '{terminal.TerminalId}' restored successfully.", GetType().FullName);
                    }
                    else
                    {
                        Trace.WriteLine($"{nameof(TryRestoreSelectionAsync)}: a concurrent selection was made — discarding restored instance.", GetType().FullName);
                    }
                }
            }
            catch (Exception ex)
