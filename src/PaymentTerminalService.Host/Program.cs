...
namespace PaymentTerminalService.Host
{
    internal static class Program
    {
        ...
        {
            using (var mutex = new Mutex(initiallyOwned: true, name: @"Global\PaymentTerminalService.Host", out bool createdNew))
            {
                if (!createdNew)
                {
                    Trace.WriteLine($"{AppName} already running, exiting.", typeof(Program).FullName);
                    return 2;
                }
